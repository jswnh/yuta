...
export default function NotificationsIndex({ notifications }: NotificationsIndexProps) {
    ...
    const getIcon = (type?: string) => {
        if (type?.includes('agreement')) return <FileText className="w-5 h-5 text-teal-600 dark:text-teal-400" />;
        if (type?.includes('transaction') || type?.includes('payment')) return <CreditCard className="w-5 h-5 text-emerald-600 dark:text-emerald-400" />;
        if (type?.includes('message') || type?.includes('inquiry')) return <MessageSquare className="w-5 h-5 text-cyan-600 dark:text-cyan-400" />;
        if (type?.includes('verification')) return <ShieldCheck className="w-5 h-5 text-amber-600 dark:text-amber-400" />;
            ...
            <div className="flex flex-col gap-6 p-6 max-w-4xl mx-auto">
                <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-800">
                    ...
                    <button
                        ...
                        disabled={markingAll}
                        className="px-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 cursor-pointer disabled:opacity-50 shadow-sm"
                    >
                ...
                <div className="space-y-3">
                    {notifications.data.length > 0 ? (
                        notifications.data.map((item) => {
                            ...
                            const actionUrl = item.data?.link || item.data?.action_url ? String(item.data?.link || item.data?.action_url) : null;
                            ...
                            return (
                                <div
                                    ...
                                    className={`p-4 rounded-2xl border transition-all flex items-start justify-between gap-4 ${
                                        isUnread ? 'bg-emerald-50/40 dark:bg-slate-900 border-emerald-300 dark:border-emerald-500/30 shadow-sm' : 'bg-white dark:bg-slate-950/60 border-slate-200 dark:border-slate-800/80 opacity-80'
                                    }`}
                                >
                                    ...
                                    <div className="flex items-center gap-2 shrink-0">
                                        {actionUrl && (
                                            <Link
                                                ...
                                                className="px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 shadow-sm"
                                            >
                                                <span>View</span>
