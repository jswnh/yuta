...
export default function Dashboard({
    ...
    return (
        <AppLayout breadcrumbs={[{ title: 'Seller Dashboard', href: '/dashboard' }]}>
            <Head title="Seller Dashboard — Yuta" />

            <div className="flex flex-col gap-8 p-6">
                {/* TOP HEADER */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white">Seller Analytics & Overview</h1>
                    ...
                    <div className="flex items-center gap-3">
                        <Link
                            ...
                            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold text-xs shadow-lg shadow-emerald-500/20 transition-all hover:scale-105"
                        >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                        ...
                        <div className="flex items-baseline justify-between">
                            <span className="text-3xl font-black text-white">{engagement.total_views.toLocaleString()}</span>
                    ...
                    <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                        ...
                        <div className="flex items-baseline justify-between">
                            <span className="text-3xl font-black text-white">{engagement.total_favorites.toLocaleString()}</span>
                    ...
                    <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                        ...
                        <div className="flex items-baseline justify-between">
                            <span className="text-2xl font-black text-emerald-400">{formatPhp(deals.total_transaction_value)}</span>
                ...
                <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-4">
                    <div className="flex items-center justify-between">
                        <div>
                            <h2 className="text-lg font-bold text-white">Deal & Agreement Pipeline</h2>
                            <p className="text-slate-400 text-xs mt-0.5">Summary of pending proposals and in-progress property transactions.</p>
                        </div>
                        <Link href="/agreements" className="text-xs font-bold text-emerald-400 hover:text-emerald-300 flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xl font-black text-white">{deals.pending_agreements}</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xl font-black text-white">{deals.active_agreements}</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xl font-black text-white">{deals.completed_transactions}</span>
                            </div>
                        </div>
                        ...
                        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800/80">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-lg font-black text-amber-300">{formatPhp(deals.pending_transaction_value)}</span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-6 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-bold text-white">Top Performing Properties</h2>
                            <Link href="/listings" className="text-xs text-slate-400 hover:text-white">View All</Link>
                        </div>

                        <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden divide-y divide-slate-800">
                            {topListings.length > 0 ? (
                                topListings.map((listing) => (
                                    <div key={listing.listing_id} className="p-4 flex items-center justify-between gap-4 hover:bg-slate-800/40 transition-colors">
                                        <div className="flex items-center gap-3 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <Link href={`/properties/${listing.slug}`} className="font-bold text-sm text-white hover:text-emerald-400 truncate block">
                                                    {listing.title}
                                                </Link>
                                        ...
                                        <div className="flex items-center gap-4 text-xs text-slate-400 shrink-0">
                                            <span className="flex items-center gap-1"><Eye className="w-3.5 h-3.5 text-cyan-400" /> {listing.view_count || 0}</span>
                                            <span className="flex items-center gap-1"><Heart className="w-3.5 h-3.5 text-rose-400" /> {listing.favorites_count || 0}</span>
                                        </div>
                                    </div>
                                ))
                            ) : (
                    ...
                    <div className="lg:col-span-6 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-bold text-white">Recent Agreements & Deals</h2>
                            <Link href="/agreements" className="text-xs text-slate-400 hover:text-white">View All</Link>
                        </div>

                        <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden divide-y divide-slate-800">
                            {recentAgreements.length > 0 ? (
                                recentAgreements.map((agreement) => (
                                    <div key={agreement.id} className="p-4 flex items-center justify-between gap-4 hover:bg-slate-800/40 transition-colors">
                                        <div className="min-w-0">
                                            <div className="flex items-center gap-2">
                                                <span className="font-mono text-xs text-slate-400 font-bold">{agreement.agreement_number}</span>
                                        ...
                                        <div className="text-right shrink-0">
                                            ...
                                            <Link href={`/agreements/${agreement.id}`} className="text-[11px] text-slate-400 hover:text-white underline">
                                                View Deal
                                            </Link>
                                        </div>
                                    </div>
