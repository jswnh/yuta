...
export default function AgreementsIndex({ agreements, filters = {} }: AgreementsIndexProps) {
    ...
    return (
        <>
            <Head title="Deal Agreements" />

            <div className="flex flex-col gap-6 p-6">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 dark:text-white">Land & Property Agreements</h1>
                        ...
                        {[
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer ${
                                    tab === t.id ? 'bg-emerald-600 text-white dark:bg-emerald-500 dark:text-slate-950 font-bold shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                                }`}
                            >
                                {t.label}
                            </button>
                        ...
                        {[
                            ...
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-xl transition-all cursor-pointer ${
                                    status === s.id ? 'bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
                                }`}
                            >
                                {s.label}
                            </button>
                ...
                <div className="space-y-4">
                    {agreements.data.length > 0 ? (
                            ...
                            >
                                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-2">
                                                <span className="font-mono text-xs text-slate-500 dark:text-slate-400 font-bold">{item.agreement_number}</span>
                                                <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                                                    item.status === 'accepted' ? 'bg-teal-500/10 text-teal-700 dark:text-teal-400 border border-teal-500/20' :
                                                    item.status === 'pending' || item.status === 'proposed' ? 'bg-amber-500/10 text-amber-700 dark:text-amber-400 border border-amber-500/20' :
                                                    item.status === 'completed' ? 'bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/20' :
                                                    item.status === 'rejected' ? 'bg-rose-500/10 text-rose-700 dark:text-rose-400 border border-rose-500/20' :
                                            ...
                                            <div className="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                                                <span>Buyer: <strong className="text-slate-700 dark:text-slate-300 font-medium">{item.buyer?.name}</strong></span>
                                                <span>•</span>
                                                <span>Seller: <strong className="text-slate-700 dark:text-slate-300 font-medium">{item.seller?.name}</strong></span>
                                                <span>•</span>
                                                <span>Payment: <strong className="capitalize text-slate-700 dark:text-slate-300 font-medium">{item.payment_type?.replace('_', ' ')}</strong></span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between sm:justify-end gap-6 sm:text-right">
                                        <div>
                                            <span className="text-[10px] uppercase font-bold text-slate-400 block">Agreed Amount</span>
                        ...
                        <div className="p-12 text-center rounded-3xl bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 shadow-sm">
                            ...
                            <h3 className="text-base font-bold text-slate-900 dark:text-white">No agreements found</h3>
