...
export default function MarketplaceIndex({ listings, filters = {} }: MarketplaceIndexProps) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
            <Head title="Public Marketplace — Yuta" />
            ...
            <header className="sticky top-0 z-50 backdrop-blur-xl bg-slate-950/85 border-b border-slate-800">
                <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
                    <Link href="/" className="flex items-center gap-2">
                        ...
                        <span className="font-black text-xl text-white">Yuta<span className="text-emerald-400">.</span></span>
                    </Link>
                    <div className="flex items-center gap-3">
                        <Link href="/" className="text-xs text-slate-400 hover:text-white">Home</Link>
                        <Link href="/billing" className="px-3 py-1.5 rounded-full bg-emerald-500 text-slate-950 font-bold text-xs">Seller</Link>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-white">Property Marketplace</h1>
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-xs font-semibold cursor-pointer ${viewMode === 'grid' ? 'bg-emerald-500 text-slate-950' : 'bg-slate-900 text-slate-400'}`}
                        >
                            Grid View
                        </button>
                        <button
                            onClick={() => setViewMode('map')}
                            className={`px-4 py-2 rounded-full text-xs font-semibold cursor-pointer ${viewMode === 'map' ? 'bg-emerald-500 text-slate-950' : 'bg-slate-900 text-slate-400'}`}
                        >
                            Interactive Map
                        </button>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                    {/* FILTERS SIDEBAR */}
                    <div className="lg:col-span-1 space-y-6 bg-slate-900/60 p-6 rounded-3xl border border-slate-800 h-fit">
                        <form onSubmit={(e) => { e.preventDefault(); applyFilters({ search }); }} className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-slate-950 border border-slate-800 rounded-2xl pl-9 pr-3 py-2 text-sm text-white focus:outline-none focus:border-emerald-500"
                            />
                        </form>

                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase block mb-2">Land Type</label>
                            <select
                                value={landType}
                                onChange={(e) => { setLandType(e.target.value); applyFilters({ land_type: e.target.value === 'all' ? '' : e.target.value }); }}
                                className="w-full bg-slate-950 border border-slate-800 rounded-2xl px-3 py-2 text-sm text-white"
                            >
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase block mb-2">Title Status</label>
                            <select
                                value={titleStatus}
                        ...
                        <div>
                            <label className="text-xs font-bold text-slate-400 uppercase block mb-2">Sort By</label>
                            <select
                                value={sort}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                                {listings.data.length > 0 ? (
                                    ...
                                    <div className="col-span-full py-16 text-center rounded-3xl bg-slate-900/50 border border-slate-800">
                                        ...
                                        <h4 className="text-lg font-bold text-white">No listings match your search</h4>
                                        <p className="text-slate-400 text-xs mt-1">Try changing or clearing your filters.</p>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {listings.last_page > 1 && (
                            <div className="mt-10 flex items-center justify-center gap-2">
                                {listings.current_page > 1 && (
                                    <Link href={`/marketplace?page=${listings.current_page - 1}`} className="px-3 py-2 rounded-xl bg-slate-900 text-xs">
                                        <ChevronLeft className="w-4 h-4" />
                                    </Link>
                                )}
                                <span className="text-xs text-slate-400">Page {listings.current_page} of {listings.last_page}</span>
